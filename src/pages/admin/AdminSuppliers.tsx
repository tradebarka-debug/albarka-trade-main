...
const defaultForm = { company_name: "", category: "Fournisseur", country: "Burkina Faso", city: "", email: "", telephone: "", whatsapp: "", website: "", description: "", logo: "", status: "active", certified: false, scope: "local" as "local" | "international" };
...
const AdminSuppliers = () => {
  ...
  const filteredSuppliers = useMemo(() => scopeFilter === "all" ? suppliers : suppliers.filter((supplier) => ((supplier as any).scope ?? "local") === scopeFilter), [suppliers, scopeFilter]);

  return <div className="space-y-6 p-6 md:p-8"><Card><CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><CardTitle>Gestion des fournisseurs</CardTitle><p className="text-sm text-muted-foreground">{filteredSuppliers.length} / {supplierCount} fournisseur(s) enregistré(s)</p></div><div className="flex flex-wrap items-center gap-2"><div className="flex rounded-lg border p-1"><button type="button" onClick={() => setScopeFilter("all")} className={`rounded-md px-3 py-1.5 text-sm ${scopeFilter === "all" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Tous</button><button type="button" onClick={() => setScopeFilter("local")} className={`rounded-md px-3 py-1.5 text-sm ${scopeFilter === "local" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Locaux</button><button type="button" onClick={() => setScopeFilter("international")} className={`rounded-md px-3 py-1.5 text-sm ${scopeFilter === "international" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Internationaux</button></div><Button onClick={openCreate}>Ajouter un fournisseur</Button></div></CardHeader><CardContent>{loading ? <p className="text-muted-foreground">Chargement…</p> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Entreprise</th><th className="p-2">Catégorie</th><th className="p-2">Pays / Ville</th><th className="p-2">Portée</th><th className="p-2">Contact</th><th className="p-2">Statut</th><th className="p-2 text-right">Actions</th></tr></thead><tbody>{filteredSuppliers.map((supplier) => <tr key={supplier.id} className="border-b align-top"><td className="p-2 font-medium">{supplier.company_name}</td><td className="p-2">{supplier.category}</td><td className="p-2">{supplier.country} / {supplier.city}</td><td className="p-2">{(supplier as any).scope === "international" ? "🌍 International" : "📍 Local"}</td><td className="p-2"><div>{supplier.telephone}</div><div>{supplier.email}</div></td><td className="p-2"><div>{supplier.status === "active" ? "Public" : "Masqué"}</div>{supplier.certified && <span className="font-semibold text-yellow-500">✓ Certifié</span>}</td><td className="space-x-2 p-2 text-right"><Button variant="outline" size="sm" onClick={() => openEdit(supplier)}>Modifier</Button><Button variant="destructive" size="sm" onClick={() => void deleteSupplier(supplier.id)}>Supprimer</Button></td></tr>)}</tbody></table></div>}</CardContent></Card>
    <Dialog open={dialogOpen} onOpenChange={(open) => open ? setDialogOpen(true) : closeDialog()}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{editingId ? "Modifier le fournisseur" : "Ajouter un fournisseur"}</DialogTitle></DialogHeader><div className="grid gap-4 md:grid-cols-2"><Field label="Entreprise *"><Input value={form.company_name} onChange={(e) => update("company_name", e.target.value)} /></Field><Field label="Catégorie"><Input value={form.category} onChange={(e) => update("category", e.target.value)} /></Field><Field label="Pays"><Input value={form.country} onChange={(e) => update("country", e.target.value)} /></Field><Field label="Ville"><Input value={form.city} onChange={(e) => update("city", e.target.value)} /></Field><Field label="Téléphone"><Input value={form.telephone} onChange={(e) => update("telephone", e.target.value)} /></Field><Field label="WhatsApp"><Input value={form.whatsapp} onChange={(e) => update("whatsapp", e.target.value)} /></Field><Field label="Email"><Input type="email" value={form.email} onChange={(e) => update("email", e.target.value)} /></Field><Field label="Site web"><Input type="url" value={form.website} onChange={(e) => update("website", e.target.value)} /></Field><Field label="Portée" className="md:col-span-2"><div className="flex rounded-lg border p-1 w-fit"><button type="button" onClick={() => update("scope", "local")} className={`rounded-md px-4 py-1.5 text-sm ${form.scope === "local" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>📍 Local</button><button type="button" onClick={() => update("scope", "international")} className={`rounded-md px-4 py-1.5 text-sm ${form.scope === "international" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>🌍 International</button></div></Field><div className="space-y-2 md:col-span-2"><Label>Logo du fournisseur</Label>{imagePreview ? <div className="relative h-32 w-32 overflow-hidden rounded-lg border"><img src={imagePreview} alt="Aperçu du logo" className="h-full w-full object-contain" /><Button type="button" variant="destructive" size="icon" className="absolute right-2 top-2" onClick={removeImage}><X className="h-4 w-4" /></Button></div> : <button type="button" onClick={() => fileInputRef.current?.click()} className="flex h-32 w-32 flex-col items-center justify-center rounded-lg border-2 border-dashed text-muted-foreground hover:border-primary"><ImageIcon className="mb-2 h-9 w-9" /><span className="text-xs">JPG, PNG, WebP<br />5 Mo maximum</span></button>}<input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={selectImage} /></div><Field label="Description" className="md:col-span-2"><Textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={4} /></Field><Toggle label="Statut public" description="Visible sur la page fournisseurs" checked={form.status === "active"} onChange={(checked) => update("status", checked ? "active" : "inactive")} /><Toggle label="Fournisseur certifié" description="Afficher le badge certifié sur la page fournisseurs" checked={form.certified} onChange={(checked) => update("certified", checked)} /></div><div className="mt-4 flex justify-end gap-2"><Button variant="outline" onClick={closeDialog} disabled={saving}>Annuler</Button><Button onClick={() => void saveSupplier()} disabled={saving}>{saving ? "Sauvegarde…" : "Sauvegarder"}</Button></div></DialogContent></Dialog>
  </div>;
};
