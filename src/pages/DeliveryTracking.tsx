...
import { CheckCircle2, ChevronDown, MapPin, PackageSearch, Phone, RefreshCw, Search, Ticket, Truck } from "lucide-react";
...
function OrderCard({ saved, result, loading }: { saved: SavedOrder; result?: TrackingResult; loading: boolean }) {
  ...
  return <article className="rounded-2xl border bg-card p-5 shadow-sm">
    ...
    {result && <section className="mt-5" aria-label="Avancement de la commande"><div className="flex items-center justify-between gap-3 text-sm"><span className="font-semibold">Progression</span><span className="text-muted-foreground">{Math.max(0, progress.step) + 1}/{steps.length}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full transition-all ${progress.blocked ? "bg-destructive" : finished ? "bg-green-600" : "bg-primary"}`} style={{ width: `${progress.step < 0 ? 4 : Math.min(100, ((progress.step + 1) / steps.length) * 100)}%` }} /></div>{!progress.blocked && !finished && progress.step >= 0 && progress.step + 1 < steps.length && <p className="mt-2 text-sm text-muted-foreground">Prochaine étape : <span className="font-medium text-foreground">{steps[progress.step + 1]}</span></p>}<details className="group mt-3"><summary className="flex cursor-pointer list-none items-center gap-2 text-sm font-medium text-primary"><ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" />Voir toutes les étapes</summary><ol className="mt-3 grid gap-2 sm:grid-cols-2">{steps.map((label, index) => <li key={label} aria-current={index === progress.step ? "step" : undefined} className={`flex items-center gap-2 rounded-lg p-2 text-sm ${index === progress.step ? "bg-primary/10 font-semibold text-primary" : index < progress.step ? "text-green-700" : "text-muted-foreground"}`}><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-xs">{index < progress.step || (finished && index === progress.step) ? <CheckCircle2 className="h-4 w-4" aria-label="Étape terminée" /> : index + 1}</span>{label}</li>)}</ol></details></section>}
